...
export default function App() {
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      const custs = await api("customers?select=*");
      const hist = await api("history?select=*");
      const map = {};
  ...
  const showToast = (msg, color = "#2d6a3f") => { setToast({ msg, color }); setTimeout(() => setToast(null), 2600); };
  const getBalance = (cid) => { const h = historyMap[cid] || []; return h.length ? h[h.length - 1].balance : 0; };
  ...
  const applyTx = async (customerId, type, amount, note, date) => {
    ...
    const entry = { id: crypto.randomUUID(), customer_id: customerId, date: date || today(), type, amount, note: note || (type === "charge" ? "충전" : "차감"), balance: newBal, created_at: new Date().toISOString() };
    await api("history", "POST", entry);
    setHistoryMap(prev => ({ ...prev, [customerId]: [...(prev[customerId] || []), entry] }));
    const c = customers.find(x => x.id === customerId);
    return `${c?.name}님 ${type === "charge" ? "+" : "-"}${fmt(amount)}원 → 잔액 ${fmt(newBal)}원`;
  };

  const addCustomerFn = async (name, phone, chargeAmt, chargeDate) => {
    const newC = { id: crypto.randomUUID(), name, phone: phone || null, created_at: new Date().toISOString() };
    await api("customers", "POST", newC);
    setCustomers(prev => [...prev, newC]);
    setHistoryMap(prev => ({ ...prev, [newC.id]: [] }));
    if (chargeAmt > 0) await applyTx(newC.id, "charge", chargeAmt, "선결제", chargeDate);
  ...
  const applyModal = async () => {
    ...
    showToast(modal.type === "charge" ? `${fmt(amt)}원 충전 완료` : `${fmt(amt)}원 차감 완료`, modal.type === "charge" ? "#1a5f8a" : "#8a3a2a");
  ...
  const sendChat = async () => {
    ...
    setAiLoading(true);
    const summary = customers.map(c => ({ id: c.id, name: c.name, phone: c.phone, balance: getBalance(c.id) }));
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `한식당 선결제 장부 AI. JSON만 반환.\n고객:${JSON.stringify(summary)}\n오늘:${today()}\nJSON:{"action":"none"|"charge"|"deduct"|"add_customer"|"query","customerId":ID또는null,"customerName":"이름","amount":금액또는null,"date":"YYYY-MM-DD","note":"메모","newCustomerCharge":0,"message":"한국어응답"}\n부분이름매칭, 5만→50000`,
      ...
      if (parsed.action === "charge" && parsed.customerId && parsed.amount) {
        toastMsg = await applyTx(parsed.customerId, "charge", parsed.amount, parsed.note, parsed.date); toastColor = "#1a5f8a";
        finalMsg += `\n\n📅 ${fmtDate(parsed.date)}`;
        ...
        else { toastMsg = await applyTx(parsed.customerId, "use", parsed.amount, parsed.note, parsed.date); toastColor = "#8a3a2a"; finalMsg += `\n\n📅 ${fmtDate(parsed.date)}`; }
      } else if (parsed.action === "add_customer" && parsed.customerName) {
        toastMsg = await addCustomerFn(parsed.customerName, null, parsed.newCustomerCharge || 0, parsed.date);
      }
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#1e1208", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>{[0,0.2,0.4].map(d => <div key={d} style={{ width: 8, height: 8, borderRadius: "50%", background: "#c8a060", animation: `pulse 1.2s ${d}s infinite` }} />)}</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#faf8f5", fontFamily: "'Noto Sans KR','Apple SD Gothic Neo',sans-serif", color: "#1e1810", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      ...
      {toast && <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: toast.color, color: "#fff", padding: "11px 22px", borderRadius: 30, fontWeight: 700, fontSize: 13, zIndex: 9999, boxShadow: "0 4px 20px rgba(0,0,0,0.22)", whiteSpace: "nowrap", animation: "fadeUp 0.3s ease" }}>{toast.msg}</div>}
      ...
      {view === "list" && (<>
        <div style={{ background: "linear-gradient(160deg,#1e1208,#3a2010)", padding: "28px 20px 20px" }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="이름 또는 전화번호 검색..." style={{ width: "100%", padding: "11px 16px", borderRadius: 12, border: "none", background: "rgba(255,255,255,0.12)", color: "#fff", fontSize: 14 }} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", background: "#2a1a0a", padding: "12px 0" }}>
          {[{ label: "전체 고객", value: `${customers.length}명` }, { label: "잔액 있음", value: `${customers.filter(c => getBalance(c.id) > 0).length}명` }, { label: "총 잔액", value: `${fmt(customers.reduce((s,c) => s + getBalance(c.id), 0))}원` }].map(item => (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#9a8a7a", marginTop: 2 }}>
                      {c.phone && <span style={{ marginRight: 6 }}>📱 {c.phone}</span>}
                      {lastCharge ? `충전 ${fmtDateShort(lastCharge.date)}` : ""}{lastCharge && lastUse ? " · " : ""}{lastUse ? `이용 ${fmtDateShort(lastUse.date)}` : ""}{!lastCharge && !lastUse && !c.phone ? "내역 없음" : ""}
                    </div>
                  </div>
                </div>
      ...
      {view === "add" && (<>
        ...
        <div style={{ padding: 20 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
            ...
            <div style={{ height: 18 }} />
            <label style={{ fontSize: 12, color: "#7a6a5a", fontWeight: 700, display: "block" }}>전화번호 <span style={{ fontWeight: 400, color: "#b0a090" }}>(선택)</span></label>
            <input value={newPhone} onChange={e => setNewPhone(e.target.value)} placeholder="예: 010-1234-5678" type="tel" style={{ width: "100%", marginTop: 8, padding: "13px 16px", border: "2px solid #e8e0d8", borderRadius: 12, fontSize: 16, background: "#faf8f5", display: "block" }} />
            <div style={{ height: 18 }} />
            ...
            <button onClick={async () => { if (!newName.trim()) return; await addCustomerFn(newName.trim(), newPhone.trim(), parseInt(newCharge)||0, newChargeDate); showToast(`${newName.trim()} 고객 등록 완료`); setNewName(""); setNewPhone(""); setNewCharge(""); setNewChargeDate(today()); setView("list"); }} disabled={!newName.trim()}
              style={{ width: "100%", marginTop: 24, padding: "15px 0", background: newName.trim() ? "linear-gradient(135deg,#c8a060,#8a5a10)" : "#d8d0c8", border: "none", borderRadius: 14, color: "#fff", fontSize: 16, fontWeight: 700 }}>고객 등록하기</button>
          </div>
        </div>
      </>)}
          ...
          <div style={{ background: "linear-gradient(160deg,#1e1208,#3a2010)", padding: "28px 20px 24px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {selected.phone && <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, marginTop: 2 }}>📱 {selected.phone}</div>}
              </div>
              <button onClick={async () => { await api(`customers?id=eq.${selected.id}`, "DELETE"); setCustomers(prev => prev.filter(c => c.id !== selected.id)); setView("list"); showToast("고객 삭제 완료", "#8a3a2a"); }}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000 }} onClick={() => setModal(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", width: "100%", maxWidth: 480, borderRadius: "24px 24px 0 0", padding: "28px 24px 44px" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              {[10000,30000,50000,100000].map(v => <button key={v} onClick={() => setForm(f=>({...f,amount:String((parseInt(f.amount)||0)+v)}))} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "1px solid #d8d0c8", background: "#f7f5f2", fontSize: 12, color: "#5a4a3a", fontWeight: 600 }}>+{v/10000}만</button>)}
            </div>
